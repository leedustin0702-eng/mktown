...
// ==========================================
// ⚙️ 시즌 및 콘텐츠 설정 스위치 (이 부분만 띡 바꾸시면 됩니다!)
// ==========================================
const CURRENT_SEASON = "시즌1";
const IS_PPUROPA_OPEN = false; // true로 바꾸면 뿌로파 탭의 자물쇠가 풀립니다!
...
export default function MKTOWNPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        const [resMain, resCup, resChamps, resFish, resRopa] = await Promise.all([
  ...
  const handleStartPinball = () => {
    ...
    const roll = () => {
      ...
      if (count < maxCount) { delay += 6; setTimeout(roll, delay); }
      else { setPbRolling(false); const winner = arr[Math.floor(Math.random() * arr.length)]; setPbResult(winner); setPbCurrent(winner); }
    };
    roll();
  };
  ...
  const traceLadder = (startCol: number) => {
    ...
    for (let r = 0; r < LADDER_ROWS; r++) {
       path.push({x: c, y: r + 1});
       if (c < ladderCols - 1 && ladderLines[r][c]) { c++; path.push({x: c, y: r + 1}); } 
       else if (c > 0 && ladderLines[r][c-1]) { c--; path.push({x: c, y: r + 1}); }
    }
  ...
  const renderPlayer = (p: any, type: 'tourney' | 'fishman') => {
    ...
    return (
        <div key={p.name} className="flex items-center gap-3 bg-[#050a08] p-3 rounded-xl border border-emerald-900/20 hover:border-emerald-500/30 transition-colors group cursor-pointer" onClick={() => { setSearchInput(streamer?.fcoNickname || p.name); handleSearch(streamer?.fcoNickname || p.name); setActiveTab('ranking'); }}>
             ...
             {type === 'tourney' && p.rank && (
                 <div className="ml-auto flex items-center justify-center w-8 h-8 rounded-full bg-slate-800/50 border border-slate-700 font-black text-amber-400 shadow-inner">
                     {p.rank}
                 </div>
             )}
        </div>
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-emerald-900/50 pb-6">
          ...
          <nav className="flex space-x-2 overflow-x-auto pb-2 w-full md:w-auto [&::-webkit-scrollbar]:h-1 [&::-webkit-scrollbar-thumb]:bg-emerald-900/50">
            {[
              ...
              <button key={tab.id} onClick={() => { setActiveTab(tab.id); if (tab.id === 'ranking') setSearchResult(null); }} className={`flex items-center gap-2 px-5 py-3 rounded-lg font-bold text-sm transition-all whitespace-nowrap border ${ activeTab === tab.id ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.15)]' : 'bg-transparent text-slate-500 border-transparent hover:text-emerald-200 hover:bg-emerald-900/20' }`}>
          ...
          <div className="animate-fadeIn space-y-6">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-3xl font-black text-white flex items-center gap-3">
                <span className="text-emerald-500">🏆</span> 진행 중인 대회 및 콘텐츠
              </h2>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-[#0a120e] border border-blue-900/50 rounded-3xl p-6 shadow-2xl">
                <h3 className="text-xl font-black text-blue-400 mb-5 pb-3 border-b border-blue-900/30 flex items-center gap-2">
                  ⚽ 미니 뿌드컵 {CURRENT_SEASON}
                </h3>
              ...
              <div className="bg-[#0a120e] border border-amber-900/50 rounded-3xl p-6 shadow-2xl">
                <h3 className="text-xl font-black text-amber-400 mb-5 pb-3 border-b border-amber-900/30 flex items-center gap-2">
                  🔥 미니 뿌챔스 {CURRENT_SEASON}
                </h3>
              ...
              <div className="bg-[#0a120e] border border-cyan-900/50 rounded-3xl p-6 shadow-2xl">
                <h3 className="text-xl font-black text-cyan-400 mb-5 pb-3 border-b border-cyan-900/30 flex items-center gap-2">
                  🌊 어인섬 도장깨기
                </h3>
              ...
              <div className="relative bg-[#0a120e] border border-purple-900/50 rounded-3xl p-6 shadow-2xl overflow-hidden">
                <h3 className="text-xl font-black text-purple-400 mb-5 pb-3 border-b border-purple-900/30 flex items-center gap-2">
                  🚧 미니 뿌로파 {CURRENT_SEASON}
                </h3>
        ...
        {activeTab === 'ranking' && (
          <div className="space-y-10 animate-fadeIn">
            <div className="bg-[#0a120e] border border-emerald-900/50 rounded-2xl p-6 shadow-2xl relative overflow-hidden">
              ...
              <h2 className="text-lg font-bold text-emerald-300 mb-4 flex items-center gap-2"><span className="animate-pulse">🟢</span> FC 온라인 구단주 전적 검색</h2>
              ...
              {isLoading && (
                <div className="mt-5 flex items-center gap-4">
                  <div className="flex-1 h-1.5 bg-emerald-950 rounded-full overflow-hidden"><div className="h-full bg-emerald-500 transition-all duration-300 shadow-[0_0_10px_rgba(16,185,129,1)]" style={{ width: `${progress}%` }}></div></div>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 animate-fadeIn">
                <div className="xl:col-span-1 space-y-6">
                  <div className="bg-[#0a120e] border border-emerald-900/50 rounded-3xl p-8 flex flex-col items-center text-center shadow-2xl relative">
                    ...
                    <h2 className="text-3xl font-black text-white flex flex-col items-center gap-2 mt-2">
                      <div className="flex items-center gap-2">{searchResult.name}{nexonBasic?.level && <span className="text-[10px] bg-emerald-900/50 text-emerald-400 px-2 py-0.5 rounded border border-emerald-800 align-middle">Lv.{nexonBasic.level}</span>}</div>
                      {searchResult.isLive && (<span className="flex items-center gap-1.5 bg-red-950/40 px-3 py-1 rounded-full border border-red-900/50 shadow-[0_0_10px_rgba(239,68,68,0.2)] mt-1"><span className="w-2 h-2 bg-red-500 rounded-full animate-pulse shadow-[0_0_5px_rgba(239,68,68,0.8)]"></span><span className="text-[10px] font-black text-red-500 tracking-widest mt-0.5">SOOP 라이브 방송 중</span></span>)}
                    </h2>
                    ...
                    {cusStats && cusStats.total > 0 ? (
                      <div className="space-y-5">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="bg-[#050a08] p-3 rounded-xl border border-emerald-900/20 text-center"><p className="text-slate-500 text-[10px] mb-1">평균 득/실점</p><p className="text-white font-bold font-mono"><span className="text-blue-400">{cusStats.avgGF}</span> / <span className="text-red-400">{cusStats.avgGA}</span></p></div>
                          <div className="bg-[#050a08] p-3 rounded-xl border border-emerald-900/20 text-center"><p className="text-slate-500 text-[10px] mb-1">평균 점유율</p><p className="text-emerald-400 font-bold font-mono">{cusStats.avgPossession}%</p></div>
                        </div>
                      </div>
                ...
                <div className="xl:col-span-2 space-y-6">
                  <div className="bg-[#0a120e] border border-emerald-900/50 rounded-3xl p-6 md:p-8 shadow-2xl">
                    <h3 className="text-xl font-bold text-white mb-5 flex items-center gap-2 border-b border-emerald-900/30 pb-3"><span className="text-emerald-500">📝</span> 최근 경기 로그 <span className="text-xs font-normal text-emerald-600 bg-emerald-950/50 px-2 py-0.5 rounded ml-2">커스텀 1on1</span></h3>
                    {matchLogs.length > 0 ? (
                  ...
                  <div className="bg-[#0a120e] border border-emerald-900/50 rounded-3xl p-6 md:p-8 shadow-2xl">
                    <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-emerald-900/30 pb-3 mb-5 gap-2"><h3 className="text-xl font-bold text-white flex items-center gap-2"><span className="text-emerald-500">⚔️</span> 100전 기반 시트 멤버 상대전적</h3></div>
                    {h2hData.length > 0 ? (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {h2hData.map((stat, idx) => (
                          <div key={idx} className="bg-[#050a08] border border-emerald-900/30 rounded-2xl p-4 flex items-center justify-between group hover:border-emerald-500/50 transition-colors relative overflow-hidden cursor-pointer" onClick={() => { setSearchInput(stat.streamer.fcoNickname); handleSearch(stat.streamer.fcoNickname); }}>
                            <div className="absolute top-0 left-0 w-1 h-full bg-emerald-900 group-hover:bg-emerald-500 transition-colors"></div>
                            <div className="flex items-center gap-3 pl-2">
                              <img src={stat.streamer.soopId ? `https://stimg.afreecatv.com/LOGO/${stat.streamer.soopId.substring(0, 2)}/${stat.streamer.soopId}/${stat.streamer.soopId}.jpg` : 'https://via.placeholder.com/150'} alt="profile" className="w-12 h-12 md:w-14 md:h-14 shrink-0 rounded-full border border-slate-700 object-cover" onError={(e) => { e.currentTarget.src = 'https://via.placeholder.com/150?text=No+Img' }} />
                              <div><p className="font-bold text-slate-100 group-hover:text-emerald-400 transition-colors">{stat.streamer.name}</p></div>
                            </div>
                            <div className="text-right flex flex-col items-end">
                              <p className="text-lg font-black text-emerald-400">{stat.wins}승 {stat.draws}무 {stat.losses}패</p>
                              <div className="flex gap-1 mt-1">
                                {stat.recentMatches.map((res, i) => ( <span key={i} className={`w-4 h-4 flex items-center justify-center rounded text-[8px] font-bold ${res === '승' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' : res === '패' ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-slate-700/50 text-slate-300 border border-slate-600/30'}`}>{res === '승' ? 'W' : res === '패' ? 'L' : 'D'}</span> ))}
                              </div>
                            </div>
                          </div>
                        ))}
                ...
                <div className="space-y-3 flex-1 flex flex-col justify-center">
                  {(['탑', '정글', '미드', '원딜', '서포터'] as const).map((role) => (
                    <div key={role} className="flex justify-between items-center bg-blue-950/20 p-4 rounded-xl border border-blue-900/30">
                      <span className="text-blue-500/70 font-bold text-sm w-12">{role}</span><span className="font-bold text-white text-lg">{blueTeam ? blueTeam[role] : '대기 중...'}</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-3 flex-1 flex flex-col justify-center">
                  {(['탑', '정글', '미드', '원딜', '서포터'] as const).map((role) => (
                    <div key={role} className="flex justify-between items-center bg-red-950/20 p-4 rounded-xl border border-red-900/30">
                      <span className="text-red-500/70 font-bold text-sm w-12">{role}</span><span className="font-bold text-white text-lg">{redTeam ? redTeam[role] : '대기 중...'}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-[#0a120e] border border-pink-900/50 rounded-3xl p-8 shadow-[0_0_40px_rgba(236,72,153,0.1)] relative overflow-hidden">
              ...
              <div className="flex flex-col md:flex-row gap-8 relative z-10 mt-10">
                <div className="w-full md:w-1/3 flex flex-col gap-4">
                  ...
                  <button onClick={handleStartPinball} disabled={pbRolling} className="w-full py-4 bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-500 hover:to-purple-500 text-white font-black text-xl rounded-xl shadow-[0_0_20px_rgba(236,72,153,0.3)] disabled:opacity-50 transition-all">{pbRolling ? '추첨 중...' : '시작 🕹️'}</button>
                </div>
        ...
        {activeTab === 'ladder' && (
          <div className="max-w-6xl mx-auto animate-fadeIn space-y-8">
            <div className="bg-[#0a120e] border border-amber-900/50 rounded-3xl p-8 shadow-[0_0_40px_rgba(245,158,11,0.1)] relative overflow-hidden">
              ...
              <p className="text-center text-amber-500/70 text-sm mb-10">사다리를 생성하고 빈칸에 이름을 자유롭게 입력한 뒤 START 버튼을 누르세요!</p>
                  ...
                  <div className="flex justify-between mb-4 relative z-10">
                    {Array.from({length: ladderCols}).map((_, i) => (
                      <div key={`p-${i}`} className="flex-1 px-1 flex justify-center">
                        <div className="flex flex-col gap-2 w-full max-w-[90px]">
                          <input type="text" className="w-full text-center text-xs md:text-sm font-bold bg-[#050812] border-2 border-amber-900/50 hover:border-amber-500/50 rounded-lg py-1.5 focus:outline-none focus:border-amber-400 text-slate-200 transition-colors" value={ladderPlayers[i] || ''} onChange={(e) => { const newP = [...ladderPlayers]; newP[i] = e.target.value; setLadderPlayers(newP); }} placeholder={`참가${i+1}`} />
                          <button onClick={() => traceLadder(i)} className={`w-full py-1.5 text-[10px] md:text-xs font-black rounded-lg transition-all ${ladderPath && ladderPath[0].x === i ? 'bg-amber-500 text-amber-950 shadow-[0_0_10px_rgba(245,158,11,0.5)]' : 'bg-amber-900/30 text-amber-500 hover:bg-amber-400 hover:text-amber-950 border border-amber-900/50'}`}>START</button>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="relative h-[450px] w-full bg-[#050812] border border-amber-900/30 rounded-2xl overflow-hidden shadow-inner">
                    {ladderLines.length === 0 ? (
                      ...
                      <svg viewBox="0 0 1000 1000" className="w-full h-full" preserveAspectRatio="none">
                        {Array.from({length: ladderCols}).map((_, c) => ( <line key={`v-${c}`} x1={getX(c)} y1={0} x2={getX(c)} y2={SVG_H} stroke="#451a03" strokeWidth="6" /> ))}
                        {ladderLines.map((rowArr, r) => rowArr.map((hasLine, c) => hasLine && ( <line key={`h-${r}-${c}`} x1={getX(c)} y1={getY(r+1)} x2={getX(c+1)} y2={getY(r+1)} stroke="#451a03" strokeWidth="6" /> )) )}
                        {ladderPath && (
                          <polyline key={ladderAnimKey} points={ladderPath.map(p => `${getX(p.x)},${getY(p.y)}`).join(' ')} fill="none" stroke="#fbbf24" strokeWidth="10" strokeLinecap="round" strokeLinejoin="round" style={{ strokeDasharray: 30000, strokeDashoffset: 30000, animation: 'drawLineAnimation 3s linear forwards' }} />
                        )}
                      </svg>
                    )}
                  </div>
                  ...
                  <div className="flex justify-between mt-4 relative z-10">
                    {Array.from({length: ladderCols}).map((_, i) => (
                      <div key={`r-${i}`} className="flex-1 px-1 flex justify-center">
                        <input type="text" className={`w-full max-w-[90px] text-center text-xs md:text-sm font-bold bg-[#050a08] border-2 rounded-lg py-2 focus:outline-none focus:border-amber-400 transition-all ${ladderEndIdx === i ? 'border-red-500 text-red-400 bg-red-950/30 shadow-[0_0_15px_rgba(239,68,68,0.5)] scale-110' : 'border-amber-900/50 text-slate-400'}`} value={ladderResults[i] || ''} onChange={(e) => { const newR = [...ladderResults]; newR[i] = e.target.value; setLadderResults(newR); }} placeholder={`결과${i+1}`} />
                      </div>
                    ))}
                  </div>
                </div>
